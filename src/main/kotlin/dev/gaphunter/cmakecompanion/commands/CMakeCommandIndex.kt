...
/**
 * The standard CMake command set (scripting, project, and CTest commands
 * from CMake's own command reference, plus long-deprecated-but-still-valid
 * commands real CMakeLists.txt files still contain). CMake commands are
 * case-insensitive (`add_executable`, `Add_Executable`, and
 * `ADD_EXECUTABLE` are all the same command) -- lookups normalize to
 * lowercase on both sides.
 */
object CMakeCommandIndex {
    private val COMMANDS: Set<String> = setOf(
        // Scripting commands
        "block", "break", "cmake_host_system_information", "cmake_language",
        "cmake_minimum_required", "cmake_parse_arguments", "cmake_path",
        "cmake_pkg_config", "cmake_policy", "configure_file", "continue",
        "else", "elseif", "endblock", "endforeach", "endfunction", "endif",
        "endmacro", "endwhile", "execute_process", "file", "find_file",
        ...
        "include_guard", "list", "macro", "mark_as_advanced", "math",
        "message", "option", "return", "separate_arguments", "set",
        "set_directory_properties", "set_property", "site_name", "string",
        ...
        "add_test", "aux_source_directory", "build_command",
        "create_test_sourcelist", "define_property", "enable_language",
        "enable_testing", "export", "fltk_wrap_ui", "get_source_file_property",
        "get_target_property", "get_test_property", "include_directories",
        "include_external_msproject", "include_regular_expression", "install",
        "link_directories", "link_libraries", "load_cache", "project",
        "remove_definitions", "set_source_files_properties",
        "set_target_properties", "set_tests_properties", "source_group",
        ...
        // Deprecated but still real/valid, still common in the wild
    ...
    fun isKnownCommand(word: String): Boolean = word.lowercase() in COMMANDS
}
